...
If you cannot complete a task, explain why clearly."""
    ...
    async def execute_task(
        ...
        try:
            # Get LLM - use agent-specific models for better tool calling support
            if not model:
                from ..config import AGENT_MODELS
                model = AGENT_MODELS.get(provider, None)
                if not model:
                    # Fallback to original defaults if agent model not defined
                    if provider == "groq":
            ...
            llm = llm_manager.get_llm(
                ...
                temperature=0.2,
                api_keys=api_keys,
                agent_mode=True  # Use agent-specific models with tool calling support
            )
            ...
            while attempt <= max_retries:
                ...
                # Get the last AI message - handle both LangChain objects and dicts
                final_message = None
                for msg in reversed(output_messages):
                    if hasattr(msg, 'type') and msg.type == 'ai':
                        # LangChain AI message object
                        final_message = msg
                        break
                    elif isinstance(msg, dict) and msg.get('role') == 'assistant':
                        # Dictionary format from fallback
                        final_message = msg
                        break
                    elif hasattr(msg, 'content') and not hasattr(msg, 'tool_call_id'):
                        # Other LangChain message with content
                        final_message = msg
                        break
                
                # Extract content and handle both LangChain objects and dicts
                if final_message:
                    content = None
                    ...
                    if content is not None:
                        ...
                        logger.warning("No content found in final message")
                else:
                    output = "Task completed but no response generated"
...
EXECUTE THE ACTION NOW using the appropriate tool. Do not explain - ACT.""")
            ...
            # Get the last AI message for final output - handle both LangChain objects and dicts
            final_message = None
            for msg in reversed(output_messages):
                if hasattr(msg, 'type') and msg.type == 'ai':
                    # LangChain AI message object
    ...
    async def _execute_without_tools(self, llm, user_input: str, workspace_path: str, messages: List = None) -> Dict:
        """
        Enhanced prompting fallback for LLMs without native tool calling support.
        Uses comprehensive instructions to simulate tool usage capabilities.
        """
        # Create enhanced context with available tools information
        ...
        enhanced_prompt = f"""You are a professional AI coding assistant. Your task is to provide DIRECT, ACTIONABLE code solutions.

Current Workspace: {workspace_path}
{f"Previous Context: {context}" if context else ""}

CRITICAL INSTRUCTIONS - READ CAREFULLY:
- DO NOT explain what you would do - PROVIDE THE ACTUAL CODE
- DO NOT say "I would" or "we should" - WRITE THE CODE DIRECTLY
- If user asks for a function, write the complete function code
- If user asks for changes, provide the exact code to add/modify
- Be concise and practical - focus on the code solution
- Include file paths where the code should go
- Always provide working, tested-style code

Task: {user_input}

Respond with the actual code implementation and where to place it. Be direct and actionable - no explanations of what you would do, just do it."""

        try:
            # Use the LLM's direct invoke method
            result = await llm.ainvoke(enhanced_prompt)
            
            # Handle different response types and format as expected Dict
            content = ""
            ...
            logger.error(f"Error in fallback execution: {e}", exc_info=True)
            error_msg = f"Error executing request: {str(e)}. Please try rephrasing your request or check your model configuration."
            return {
    ...
    def _get_tools_description(self) -> str:
        """Generate description of available tools for enhanced prompting."""
        if not self.tools:

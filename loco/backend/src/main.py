...
async def process_with_agent(request: dict):
    ...
    try:
        # Set runtime API keys from request
        api_keys = request.get("api_keys", {})
        if api_keys:
            llm_manager.set_runtime_keys(api_keys)
...
async def execute_agentic_task(request: dict):
    ...
    if api_keys:
        llm_manager.set_runtime_keys(api_keys)
        logger.info(f"Set runtime keys in LLM manager: {list(llm_manager.runtime_api_keys.keys())}")
    else:
        logger.warning("No API keys received in request!")

...
from ..config import settings, PROVIDER_MODELS, AGENT_MODELS
    ...
    def get_llm(
        ...
        **kwargs
        ...
        Args:
            ...
            agent_mode: If True, use agent-specific models with tool calling support
        ...
        # Use agent-specific models if in agent mode and no specific model provided
        if agent_mode and not model and provider in AGENT_MODELS:
            model = AGENT_MODELS[provider]
            logger.info(f"Using agent-specific model: {model} for provider {provider}")
